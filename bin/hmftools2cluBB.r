####################################################
## Perform multi-sample segmentation and          ##
## Formats hmftools output to hatchet cluBB input ##
####################################################
...
# read RDR
COBALTs = lapply( list.files(args[2],recursive = T,pattern = "cobalt.ratio.tsv",full.names = T), read_tsv) #.T1  = read_tsv(args[4])
...
# shape input
AMBER.pcfinput = as.data.frame(lapply(AMBERs,function(x) x[,c(1,2,4)]) %>% reduce(inner_join) )
COBALT.pcfinput = as.data.frame( lapply(COBALTs,function(x) x[,c(1,2,6,7)]) %>% reduce(inner_join) )#inner_join(COBALT.T1[,c(1,2,6,7)],COBALT.T2[,c(1,2,6,7)]) ) #inner_join(COBALT.T1[,c(1,2,4)],COBALT.T2[,c(1,2,4)]) )
...
#fit
AMBER.multipcfoutput  = multipcf(AMBER.pcfinput,gamma=100,arms = AMBER.pcfinput.arms)
...
#write output in hatchet format
...
write_tsv(out_formatted,file = paste0(args[3],args[1],".bb") )
